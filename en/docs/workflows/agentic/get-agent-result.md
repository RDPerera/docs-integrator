...
A gated activity or a human task counts as still working: the instance is suspended waiting on a person, so a run that looks stalled is often one that needs a decision in the [Control Plane](../icp/human-tasks.md).
...
- [Workflow executions](../icp/executions.md) — seeing why an instance is still working.
