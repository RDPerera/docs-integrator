...
This is not the Integration Control Plane server's own API. ICP serves the console over GraphQL on port `9446` and spans every registered integration, while the API on this page is integration-local and reaches only one runtime. See [Integration Control Plane](../../manage/icp/integration-control-plane.md) for that side.
