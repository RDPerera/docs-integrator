...
3. Enter a **User name** and **User ID** for the application (e.g., `CORE-APP`).
...
5. Assign one or more **Security roles** that grant the access the connector needs — for example, a role with read/write access to **Organization administration → Setup → Legal entities**, **Organizational hierarchies**, business units, departments, and address setup (address books, formats, cities, states).
