...
4. Save the user, then assign it one or more security roles that grant access to the Trade entities you plan to use — for example, roles that cover Intrastat reporting, foreign trade, or tax compliance — via **System administration → Security → Assign users to roles**.
