...
- **Config** : An expression referencing the `auth` record, which carries the `tokenUrl`, `clientId`, and `clientSecret` configurables for the OAuth2 client-credentials grant.
